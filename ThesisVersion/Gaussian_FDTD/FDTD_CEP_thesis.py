...
Ephase = np.zeros([4,len(Xgrid)]) #Store electric field output for different phases if needed
phases =  [0.,np.pi/4,np.pi/2,(3*np.pi)/4] #Some phases
for PHS in range(len(phases)):
 ...
 for Q in range(len(Tgrid)):
  E = FuncDTD.Esolv(E,B,P,dt,dx,neProf.gauss(Xgrid,Tgrid,vphi,Tgrid[Q],tgrow[Q],twid[Q],amplitude,center,te,offset,setting='normal')/gamme)      
  E[0]=0
 ...
 Ephase[PHS] = E #Stores the electric field at the end of each simulation
