...
def gauss(x,Tgrid,vb,t,tgrow,twid,amp,cent,sig,offset,setting='normal'): 

 if setting == 'normal':
  return amp*np.exp(-(x-cent-vb*t)**2 / (vb**2*sig**2))+offset

 if setting == 'amp':
  return (tgrow*amp)*np.exp(-(x-cent-vb*t)**2 / (vb**2*sig**2))+offset

 if setting == 'wid':
  return amp*np.exp(-(x-cent-vb*t)**2 / (vb**2*twid*sig**2))+offset

 if setting == 'both':
  return (tgrow*amp)*np.exp(-(x-cent-vb*t)**2 / (vb**2*twid*sig**2))+offset
