...
def GaussForward(E,B,A0,PULSELENGTH,PULSESTART,OMEGAPRIM, KPRIM,l_0,phiS,dt,dx):
 ...
 for i in range(len(El)):
  El[i] = -OMEGAPRIM*A0*(np.sin(KPRIM*xEl[i]+phiS)*np.exp(-(xEl[i]**2/(2*l_0**2)))) + (KPRIM/OMEGAPRIM)*A0*(np.cos(KPRIM*xEl[i]+phiS)*np.exp(-(xEl[i]**2/(2*l_0**2))))*(-2*xEl[i]/(2*l_0**2))
  Bl[i] = KPRIM*A0*(np.sin(KPRIM*xBl[i]+phiS)*np.exp(-(xBl[i]**2/(2*l_0**2)))) - A0*(np.cos(KPRIM*xEl[i]+phiS)*np.exp(-(xEl[i]**2/(2*l_0**2))))*(-2*xEl[i]/(2*l_0**2))
